      ...
      class CategoriesController < Erp::Backend::BackendController
        ...
        def create
          ...
          if @category.save
            if request.xhr?
              ...
              redirect_to erp_banners.edit_backend_category_path(@category), notice: t('.success')
            end
          else
        ...
        def destroy
          ...
          respond_to do |format|
            format.html { redirect_to erp_banners.backend_categories_path, notice: t('.success') }
            format.json {
              render json: {
                'message': t('.success'),
                'type': 'success'
        ...
        def dataselect
          respond_to do |format|
            format.json {
              render json: Category.dataselect(params[:keyword], params)
            }
          end
        end
        ...
        private
          ...
          def category_params
            params.fetch(:category, {}).permit(:name, :width, :height, :image_scale, :position)
          end
      end
    end
  end
